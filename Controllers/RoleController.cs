    ...
    ) : Controller
{
    ...
    public async Task<IActionResult> CreateRole(string roleName)
    {
        try
        {
            ...
            if (result.Succeeded)
            {
                logger.LogInformation($"Role '{newRole.Name}' created successfully.");
                return RedirectToAction("ManageRoles");
            }

            // If creation failed, log errors, add error to ModelState, and return ManageRoles view
            logger.LogError($"Failed to create role '{newRole.Name}'. Errors: {string.Join(", ", result.Errors)}");
    ...
    public async Task<IActionResult> UpdateRole(string roleId, string newRoleName)
    {
        try
        {
            ...
            if (role == null)
            {
                logger.LogWarning($"Role with ID '{roleId}' not found.");
                return NotFound();
            }
            ...
            if (result.Succeeded)
            {
                logger.LogInformation($"Role '{role.Name}' updated successfully.");
                return RedirectToAction("ManageRoles");
            }

            // If update failed, log errors, add error to ModelState, and return ManageRoles view
            logger.LogError($"Failed to update role '{role.Name}'. Errors: {string.Join(", ", result.Errors)}");
    ...
    public async Task<IActionResult> DeleteRole(string roleId)
    {
        try
        {
            ...
            if (result.Succeeded)
            {
                logger.LogInformation($"Role '{role.Name}' deleted successfully.");
                return RedirectToAction("ManageRoles");
            }

            // If deletion failed, log errors, add error to ModelState, and return ManageRoles view
            logger.LogError($"Failed to delete role '{role.Name}'. Errors: {string.Join(", ", result.Errors)}");
